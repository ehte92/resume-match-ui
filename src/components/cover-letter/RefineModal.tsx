...
export const RefineModal = ({ isOpen, onClose, coverLetter, onAcceptRefinement }: RefineModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white border-2 border-black shadow-2xl rounded max-w-7xl w-full max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-6 border-b-2 border-black">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Sparkles className="h-6 w-6 text-white" />
              <h2 className="text-2xl font-bold text-white">Refine with AI</h2>
            </div>
            <button
              onClick={handleClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
          ...
          {!refinementResult && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="border-2 border-black rounded p-4 max-h-64 overflow-y-auto bg-gray-50">
                  <p className="text-sm whitespace-pre-wrap">{coverLetter.cover_letter_text}</p>
                </div>
              </div>

              {/* Actions */}
              <div className="flex gap-3 pt-4 border-t-2 border-black">
                <Button type="button" variant="outline" onClick={handleClose} disabled={isRefining} className="flex-1">
                  Cancel
                </Button>
                <Button onClick={handleRefine} disabled={isRefining || instructionLength < 10} className="flex-1">
                  {isRefining ? (
                    <>
          ...
          {refinementResult && (
            <div className="space-y-4">
              ...
              <div className="bg-purple-50 border-2 border-purple-200 rounded p-4">
                <p className="text-xs font-medium text-purple-900 mb-1">REFINEMENT INSTRUCTION:</p>
                <p className="text-sm text-purple-900">{refinementResult.refinement_instruction}</p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="border-2 border-black rounded p-3 bg-gray-50">
                  ...
                  <p className="text-lg font-bold">{coverLetter.word_count} words</p>
                </div>
                <div className="border-2 border-black rounded p-3 bg-purple-50">
                  <p className="text-xs text-muted-foreground mb-1">Refined</p>
                  <p className="text-lg font-bold">{refinementResult.word_count} words</p>
                </div>
                <div className="border-2 border-black rounded p-3 bg-gray-50">
                  <p className="text-xs text-muted-foreground mb-1">Processing Time</p>
                  <p className="text-lg font-bold">{(refinementResult.processing_time_ms / 1000).toFixed(1)}s</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label className="mb-2 flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">({coverLetter.word_count} words)</span>
                  </Label>
                  <div className="border-2 border-black rounded p-4 h-96 overflow-y-auto bg-gray-50">
                    <p className="text-sm whitespace-pre-wrap">{coverLetter.cover_letter_text}</p>
                  </div>
                </div>
                ...
                <div>
                  <Label className="mb-2 flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">({refinementResult.word_count} words)</span>
                  </Label>
                  <div className="border-2 border-purple-500 rounded p-4 h-96 overflow-y-auto bg-purple-50">
                    <p className="text-sm whitespace-pre-wrap">{refinementResult.refined_cover_letter_text}</p>
                  </div>
                </div>
              </div>
